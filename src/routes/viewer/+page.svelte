...
<script lang="ts">
  ...
  import FakeSlider from '$lib/Slider.svelte';
  ...
  let jsonError = $state('');
  let activeTab = $state('params'); // Changed default tab to params
  ...
  let parameterGroups = $state({});
  let updateParameter: ((name: string, v: number) => boolean) | null = null;
  ...
  function extractParameterGroups(spell: any) {
    const groups = {};
    for (const child of spell?.children ?? []) {
      if (child.type !== 'parametric_template') continue;
      for (const [name, p] of Object.entries(child.parameters ?? {})) {
        const gid = p.group || 'default';
        const meta = spell.ui_controls?.groups?.[gid] ?? {};
        if (!groups[gid]) groups[gid] = { 
          label: meta.label || gid, 
          order: meta.order ?? 0, 
          list: [] 
        };
        groups[gid].list.push({ name, def: { ...p }, templateId: child.id });
      }
    }
    // Sort lists to keep stable order
    for (const g of Object.values(groups)) {
      g.list.sort((a, b) => a.name.localeCompare(b.name));
    }
    parameterGroups = groups;
  }

  function setParam(name: string, v: number) {
    if (!updateParameter) return;
    
    // Update live scene
    const success = updateParameter(name, v);
    if (!success) return;
    
    // Sync local JSON text (so export/UI match)
    try {
      const doc = JSON.parse(jsonText);
      for (const child of doc.children ?? []) {
        if (child.type === 'parametric_template' && child.parameters?.[name]) {
          child.parameters[name].value = v;
          
          // Also update the parameter groups state
          for (const group of Object.values(parameterGroups)) {
            const param = group.list.find(p => p.name === name);
            if (param) {
              param.def.value = v;
              break;
            }
      ...
      jsonText = JSON.stringify(doc, null, 2);
      originalJsonText = jsonText;
      jsonModified = false;
    } catch (e) {
      console.warn('Failed to sync parameter change to JSON:', e);
    }
  }
  ...
  async function startViewer(data: unknown) {
    ...
    viewer = await start(canvas, data, false); // Disable built-in dat.gui
    updateParameter = viewer?.updateParameter ?? null;
    extractParameterGroups(data);
...
<div class="wrapper">
  ...
  <div class="sidebar" style="width:{sidebarWidth}%;">
    ...
    <div class="tab-nav">
      <button
        class="tab-btn {activeTab === 'params' ? 'active' : ''}"
    ...
    <div class="tab-content">
      {#if activeTab === 'params'}
        <div class="params-container">
          {#each Object.entries(parameterGroups).sort(([,a],[,b]) => a.order - b.order) as [gid, group]}
            <div class="param-group">
              <h4 class="group-title">{group.label}</h4>
              {#each group.list as param}
                {#if param.def.type === 'number' || param.def.type === 'integer'}
                  <FakeSlider
                    name={param.name}
                    label={param.def.label || param.name}
                    value={param.def.value}
                    min={param.def.min}
                    max={param.def.max}
                    step={param.def.step || (param.def.type === 'integer' ? 1 : 0.01)}
                    onChange={(v) => setParam(param.name, v)}
                  />
                {:else if param.def.type === 'enum'}
                  <div class="param">
                    <div class="row">
                      <span class="label">{param.def.label || param.name}</span>
                    </div>
                    <select 
                      class="enum-select"
                      value={param.def.value} 
                    ...
                    >
                      {#each param.def.options as option}
                        <option value={option}>{option}</option>
                      {/each}
                    </select>
                  </div>
                {:else if param.def.type === 'boolean'}
                  <div class="param">
                    <div class="row">
                      <span class="label">{param.def.label || param.name}</span>
                      <input
                        type="checkbox"
                        class="bool-checkbox"
                        checked={param.def.value}
                        onchange={(e) => {
                          const v = e.target.checked;
                          param.def.value = v;
                          setParam(param.name, v ? 1 : 0);
                        }}
                      />
                    </div>
                  </div>
          ...
          {#if Object.keys(parameterGroups).length === 0}
            <div class="no-params">
              <p>No parameters available for this schema.</p>
            </div>
          {/if}
        </div>
