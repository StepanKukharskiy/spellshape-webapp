            ...
            try {
                ...
                if (inputValue.trim().length < 50) {
                    ...
                    if (expandResponse.ok) {
                        ...
                        finalPrompt = expandedData.prompt;
                        console.log('Expanded prompt:', finalPrompt);
                    } else {
                        console.warn('Prompt expansion failed, using original prompt');
                    }
                }
                ...
                // Set both schema and prompt before navigation
                schema.set(generatedSchema);
                generatedPrompt.set(finalPrompt);
                
                // Navigate to viewer - chat will initialize automatically
                goto(`/viewer`);
